...
type CommentRepository interface{
	...
	GetCommentbyPhotoId(photo_id uint)([]models.Comment, error)
...
func(db *dbConnection) GetCommentbyPhotoId(photo_id uint)([]models.Comment, error){
	var Comment []models.Comment
	connection := db.connection.Where("photo_id = ?", photo_id).Preload("User").Preload("Photo").Find(&Comment)
	err := connection.Error
	if err != nil{
		return Comment, err
	}
...
func(db *dbConnection) GetCommentbyId(id uint)(models.Comment, error){
	var Comment models.Comment
	connection := db.connection.Where("id = ?", id).Preload("User").Preload("Photo").Find(&Comment)
	err := connection.Error
	if err != nil{
		return Comment, err
	}
